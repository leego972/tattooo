...
import { ENV, SOCIAL_ENV } from "./_core/env";
...
export const metaAdapter = {
  get isConfigured(): boolean {
    return !!(SOCIAL_ENV.metaAccessToken && (SOCIAL_ENV.metaPageId || SOCIAL_ENV.metaAdAccountId));
  },

  getStatus(): ChannelStatus[] {
    const statuses: ChannelStatus[] = [];

    if (SOCIAL_ENV.metaAccessToken && SOCIAL_ENV.metaPageId) {
      statuses.push({
        id: "meta_facebook",
        name: "Facebook",
        connected: true,
    ...
    if (SOCIAL_ENV.metaAccessToken && SOCIAL_ENV.metaInstagramAccountId) {
      statuses.push({
        id: "meta_instagram",
        name: "Instagram",
        connected: true,
  ...
  async postToFacebook(params: {
    ...
    try {
      ...
      const body: Record<string, string> = {
        access_token: SOCIAL_ENV.metaAccessToken,
      };

      if (params.imageUrl) {
        endpoint = `https://graph.facebook.com/v19.0/${SOCIAL_ENV.metaPageId}/photos`;
        body.url = params.imageUrl;
        body.caption = params.message;
      } else {
        endpoint = `https://graph.facebook.com/v19.0/${SOCIAL_ENV.metaPageId}/feed`;
  ...
  async postToInstagram(params: {
    ...
    try {
      ...
      const container = await apiCall(
        `https://graph.facebook.com/v19.0/${SOCIAL_ENV.metaInstagramAccountId}/media`,
        {
          method: "POST",
          headers: { "Content-Type": "application/x-www-form-urlencoded" },
      ...
      const publish = await apiCall(
        `https://graph.facebook.com/v19.0/${SOCIAL_ENV.metaInstagramAccountId}/media_publish`,
        {
          method: "POST",
          headers: { "Content-Type": "application/x-www-form-urlencoded" },
          body: new URLSearchParams({
  ...
  async createAdCampaign(params: {
    ...
    if (!SOCIAL_ENV.metaAccessToken || !SOCIAL_ENV.metaAdAccountId) {
      return { success: false, error: "Meta Ads not configured" };
    }

    try {
      const actId = SOCIAL_ENV.metaAdAccountId.startsWith("act_")
        ? SOCIAL_ENV.metaAdAccountId
        : `act_${SOCIAL_ENV.metaAdAccountId}`;
  ...
  async getMetrics(campaignId: string): Promise<PerformanceMetrics | null> {
    if (!SOCIAL_ENV.metaAccessToken) return null;

    try {
      const data = await apiCall(
        `https://graph.facebook.com/v19.0/${campaignId}/insights?fields=impressions,reach,clicks,spend,actions,ctr,cpc,cpm&access_token=${SOCIAL_ENV.metaAccessToken}`
      );
...
export const googleAdsAdapter = {
  get isConfigured(): boolean {
    return !!(
      SOCIAL_ENV.googleAdsDevToken &&
      SOCIAL_ENV.googleAdsCustomerId &&
      (SOCIAL_ENV.googleAdsClientId || SOCIAL_ENV.googleClientId) &&
      (SOCIAL_ENV.googleAdsClientSecret || SOCIAL_ENV.googleClientSecret) &&
      SOCIAL_ENV.googleAdsRefreshToken
    );
  },
  ...
  async _getAccessToken(): Promise<string> {
    const clientId = SOCIAL_ENV.googleAdsClientId || SOCIAL_ENV.googleClientId;
    const clientSecret = SOCIAL_ENV.googleAdsClientSecret || SOCIAL_ENV.googleClientSecret;
    ...
    const data = await apiCall("https://oauth2.googleapis.com/token", {
      ...
      body: new URLSearchParams({
        ...
        refresh_token: SOCIAL_ENV.googleAdsRefreshToken,
  ...
  async createSearchCampaign(params: {
    ...
    try {
      ...
      const customerId = SOCIAL_ENV.googleAdsCustomerId.replace(/-/g, "");
      const baseUrl = `https://googleads.googleapis.com/v16/customers/${customerId}`;

      const headers = {
        Authorization: `Bearer ${accessToken}`,
        "developer-token": SOCIAL_ENV.googleAdsDevToken,
...
export const xAdapter = {
  get isConfigured(): boolean {
    return !!(SOCIAL_ENV.xApiKey && SOCIAL_ENV.xApiSecret && SOCIAL_ENV.xAccessToken && SOCIAL_ENV.xAccessTokenSecret);
  },
  ...
  _generateOAuthHeader(method: string, url: string, params: Record<string, string> = {}): string {
    ...
    const oauthParams: Record<string, string> = {
      oauth_consumer_key: SOCIAL_ENV.xApiKey,
      oauth_nonce: nonce,
      oauth_signature_method: "HMAC-SHA1",
      oauth_timestamp: timestamp,
      oauth_token: SOCIAL_ENV.xAccessToken,
      oauth_version: "1.0",
      ...params,
    };
    ...
    const signingKey = `${encodeURIComponent(SOCIAL_ENV.xApiSecret)}&${encodeURIComponent(SOCIAL_ENV.xAccessTokenSecret)}`;
...
export const linkedinAdapter = {
  get isConfigured(): boolean {
    return !!(SOCIAL_ENV.linkedinAccessToken && (SOCIAL_ENV.linkedinOrgId || SOCIAL_ENV.linkedinAdAccountId));
  },
  ...
  async postToPage(params: {
    ...
    if (!SOCIAL_ENV.linkedinAccessToken || !SOCIAL_ENV.linkedinOrgId) {
      return { success: false, error: "LinkedIn not configured" };
    }

    try {
      const author = `urn:li:organization:${SOCIAL_ENV.linkedinOrgId}`;
      ...
      const data = await apiCall("https://api.linkedin.com/v2/ugcPosts", {
        method: "POST",
        headers: {
          Authorization: `Bearer ${SOCIAL_ENV.linkedinAccessToken}`,
          "Content-Type": "application/json",
  ...
  async createSponsoredCampaign(params: {
    ...
    if (!SOCIAL_ENV.linkedinAccessToken || !SOCIAL_ENV.linkedinAdAccountId) {
      return { success: false, error: "LinkedIn Ads not configured" };
    }

    try {
      const accountUrn = `urn:li:sponsoredAccount:${SOCIAL_ENV.linkedinAdAccountId}`;
  ...
  async getMetrics(campaignId: string): Promise<PerformanceMetrics | null> {
    if (!SOCIAL_ENV.linkedinAccessToken) return null;
...
export const snapchatAdapter = {
  get isConfigured(): boolean {
    return !!(SOCIAL_ENV.snapchatAccessToken && SOCIAL_ENV.snapchatAdAccountId);
  },
  ...
  async createCampaign(params: {
    ...
    if (!SOCIAL_ENV.snapchatAccessToken || !SOCIAL_ENV.snapchatAdAccountId) {
    ...
    try {
      ...
      const headers = {
        Authorization: `Bearer ${SOCIAL_ENV.snapchatAccessToken}`,
        "Content-Type": "application/json",
      };

      // Step 1: Create campaign
      const campaignResp = await apiCall(
        `${baseUrl}/adaccounts/${SOCIAL_ENV.snapchatAdAccountId}/campaigns`,
        {
          method: "POST",
          headers,
          body: JSON.stringify({
            campaigns: [
              {
                name: params.name,
                ad_account_id: SOCIAL_ENV.snapchatAdAccountId,
                status: "PAUSED",
                objective: params.objective,
      ...
      const creativeResp = await apiCall(
        `${baseUrl}/adaccounts/${SOCIAL_ENV.snapchatAdAccountId}/creatives`,
        {
          method: "POST",
          headers,
          body: JSON.stringify({
  ...
  /** Get campaign performance metrics */
  async getMetrics(campaignId: string): Promise<PerformanceMetrics | null> {
    if (!SOCIAL_ENV.snapchatAccessToken) return null;
...
const sendgridAdapter = {
  get isConfigured() {
    return !!SOCIAL_ENV.sendgridApiKey;
  },
  ...
  async sendCampaignEmail(params: {
    ...
    try {
      ...
      const response = await fetch("https://api.sendgrid.com/v3/mail/send", {
        method: "POST",
        headers: {
          Authorization: `Bearer ${SOCIAL_ENV.sendgridApiKey}`,
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          personalizations: params.recipients.map((email) => ({ to: [{ email }] })),
          from: { email: SOCIAL_ENV.sendgridFromEmail, name: SOCIAL_ENV.sendgridFromName },
  ...
  async getStats(startDate: string, endDate: string): Promise<PerformanceMetrics> {
    try {
      const response = await fetch(
        ...
        { headers: { Authorization: `Bearer ${SOCIAL_ENV.sendgridApiKey}` } }
      );
      const data = await response.json() as any[];
...
const redditAdapter = {
  get isConfigured() {
    return !!(SOCIAL_ENV.redditClientId && SOCIAL_ENV.redditClientSecret && SOCIAL_ENV.redditRefreshToken);
  },
  ...
  async getAccessToken(): Promise<string> {
    ...
    const auth = Buffer.from(`${SOCIAL_ENV.redditClientId}:${SOCIAL_ENV.redditClientSecret}`).toString("base64");
    const response = await fetch("https://www.reddit.com/api/v1/access_token", {
      ...
      body: `grant_type=refresh_token&refresh_token=${SOCIAL_ENV.redditRefreshToken}`,
    });
...
const tiktokAdapter = {
  get isConfigured() {
    return !!(SOCIAL_ENV.tiktokAccessToken && SOCIAL_ENV.tiktokAdvertiserId);
  },
  ...
  async createCampaign(params: {
    ...
    try {
      const response = await fetch("https://business-api.tiktok.com/open_api/v1.3/campaign/create/", {
        method: "POST",
        headers: {
          "Access-Token": SOCIAL_ENV.tiktokAccessToken,
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          advertiser_id: SOCIAL_ENV.tiktokAdvertiserId,
...
const pinterestAdapter = {
  get isConfigured() {
    return !!SOCIAL_ENV.pinterestAccessToken;
  },
  ...
  async createPin(params: {
    ...
    try {
      const response = await fetch("https://api.pinterest.com/v5/pins", {
        method: "POST",
        headers: {
          Authorization: `Bearer ${SOCIAL_ENV.pinterestAccessToken}`,
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          title: params.title,
          ...
          link: params.link,
          board_id: params.boardId || SOCIAL_ENV.pinterestBoardId,
          media_source: {
  ...
  async createAdCampaign(params: {
    ...
    try {
      const response = await fetch(
        `https://api.pinterest.com/v5/ad_accounts/${SOCIAL_ENV.pinterestAdAccountId}/campaigns`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${SOCIAL_ENV.pinterestAccessToken}`,
  ...
  async getAnalytics(params: {
    ...
    try {
      const response = await fetch(
        `https://api.pinterest.com/v5/ad_accounts/${SOCIAL_ENV.pinterestAdAccountId}/analytics?start_date=${params.startDate}&end_date=${params.endDate}&granularity=TOTAL&columns=SPEND_IN_MICRO_DOLLAR,IMPRESSION_1,CLICKTHROUGH_1,TOTAL_CONVERSIONS`,
        { headers: { Authorization: `Bearer ${SOCIAL_ENV.pinterestAccessToken}` } }
      );
...
export async function postToAllChannels(params: {
  ...
  // Facebook
  if (metaAdapter.isConfigured && SOCIAL_ENV.metaPageId) {
    results.meta_facebook = await metaAdapter.postToFacebook(params);
  }

  // Instagram (requires image)
  if (metaAdapter.isConfigured && SOCIAL_ENV.metaInstagramAccountId && params.imageUrl) {
  ...
  // LinkedIn
  if (linkedinAdapter.isConfigured && SOCIAL_ENV.linkedinOrgId) {
